...
//  COMPILE: cc -O3 -Wall -o eng rnt_engine_distrW.c -lgsl
...
#define AOUP //AOUP //RTP //PASSIVE

#ifdef RTP
#define PART_NAME "RTP"
#elifdef AOUP
#define PART_NAME "AOUP"
#else
#define PART_NAME "PP"
#endif
...
//#define MIN_W (-0.045)
//#define MAX_W (0.045)
//#define DW (0.00005605062275)

#define DISTR_NUM_BOXES ((int) floor((MAX_W-MIN_W)/DW))

#ifdef RTP
#define WRIGGLE_ROOM (100)
#elifdef AOUP
#define WRIGGLE_ROOM (200)
#endif
...
int flag_print_POS=0;
int flag_print_WORK=0;
...
int Ncycles(int repeat){
    if (repeat==0) return CHUNK;
    else return NUM_CYCLES;
}

int xindex_dens_work(double obs_work_cycle, double min_work, double max_work){
    double dW = (max_work-min_work)/((double)(DISTR_NUM_BOXES-WRIGGLE_ROOM)); //leaving a bit of wriggle room outside [min,max]
    int ind = floor( (obs_work_cycle-min_work)/dW + WRIGGLE_ROOM/2. );
    if(ind<0) ind = 0;
    else if(ind>=DISTR_NUM_BOXES) ind = DISTR_NUM_BOXES-1;
    
    return ind;
}

int index_dens_work(double obs_work_cycle){
    int ind = (int) floor( (obs_work_cycle-MIN_W)/DW );
    if (ind<0) ind = 0;
    else if (ind>=DISTR_NUM_BOXES) ind = DISTR_NUM_BOXES;
    return ind;
}


double xrep_dens_work(int ind, double min_work, double max_work){
    double dW = (max_work-min_work)/((double)(DISTR_NUM_BOXES-WRIGGLE_ROOM));
    if(ind>=-1 && ind<=DISTR_NUM_BOXES) return min_work+((double)(1.+2.*ind-WRIGGLE_ROOM))*dW/2.;
    else{
...
double rep_dens_work(int ind){
    if(ind>=0 && ind<DISTR_NUM_BOXES) return MIN_W + ind*DW; //min_work+((double)(1.+2.*ind-WRIGGLE_ROOM))*dW/2.;
    else{
        fprintf(stderr,"Error: index  %d out of bounds  [0, %d]\n",ind,DISTR_NUM_BOXES);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char *argv[], char *envp[]){
    ...
    //double obs_work_smooth,obs_work_CHUNK_smooth,obs_work2_smooth,
    double obs_work_cycle,theor_work_engine;
    double obs_work_engine;
    long double power,work_cycle_mom[NUM_MOMS];
    int mom;
    for(mom=0;mom<NUM_MOMS;mom++) work_cycle_mom[mom] = 0.;
    ...
    //N = NUM_REALISATIONS;
    //Ncycles = NUM_CYCLES;
    int neg_work, zero_work, pos_work;
    neg_work = zero_work = pos_work = 0;
    
    /* //the seed can be reset to the default value, but it takes one call to generate the same sequence of random numbers
     //meh, it doesn't seem to work in the general case, the RGN might have multiple steps in memory.
...

    {
        ...
        int run_distr,ind;
        //for(i=0;i<N;i++)
        double min_work,max_work;
        min_work = max_work = 0.;
        MALLOC(dens_work,DISTR_NUM_BOXES);
        for(ind=0;ind<DISTR_NUM_BOXES;ind++) dens_work[ind] = 0.;
        double dW = 0.;

        for(run_distr=0;run_distr<=flag_distr_WORK;run_distr++) {
            printf("#Info: run_distr = %d\n",run_distr);
            //printf("#GSL  %d  %lu\n",run_distr,gsl_rng_get(r));
            ...
            //engine starts here
            for(cycle=0;cycle<Ncycles(run_distr);cycle++){
                obs_work_cycle = 0.;
                ...
                if(!((cycle+1)%(50*CHUNK))) printf("#Info: cycle %d\n",cycle+1);
                ...
                if(run_distr==0){
                    //min and max density bounds are estimated in the first engine round
                    min_work = MIN(min_work,obs_work_cycle);
                    max_work = MAX(max_work,obs_work_cycle);
                } else {
                    ind = index_dens_work(obs_work_cycle);
                    dens_work[ind] ++;
                    //printf("  %d  %d  %.16LG  %.16G\n",cycle,ind,dens_work[ind],obs_work_cycle);
                    
                    work_cycle_mom[0] ++;
                    power = obs_work_cycle;
                    for(mom=1;mom<NUM_MOMS;mom++) {
                        work_cycle_mom[mom] += power;
                        power *= obs_work_cycle;
                    }

                }
                if(flag_distr_WORK && run_distr>0) if(!((cycle+1)%(200*CHUNK))) {
                    //printf("WDIS_%d  "PART_NAME"  -1  %.16G  %d  0\n",cycle+1, rep_dens_work(0), cycle+1);
                    for(ind=0;ind<DISTR_NUM_BOXES;ind++) printf("WDIS_%d  "PART_NAME"  %d  %.16G  %d  %.16LG  %.16G\n",cycle+1, ind, rep_dens_work(ind), cycle+1,dens_work[ind]/dW,(part.Dy)*(exp(-2.*final_t/(part.tau)) - 1.)/8.);
                    //printf("WDIS_%d  "PART_NAME"  %d  %.16G  %d  0\n",cycle+1, DISTR_NUM_BOXES,rep_dens_work(DISTR_NUM_BOXES), cycle+1);
                    printf("WMOM_CHUNK%d  "PART_NAME"  ",cycle+1);
                    for(mom=0;mom<NUM_MOMS;mom++) printf("%.16LG  ",work_cycle_mom[mom]);
                    printf("\n");
                }
                
                if(obs_work_cycle<0) neg_work++;
                else if(obs_work_cycle==0) zero_work++;
                else pos_work++;

            }
            
            if(run_distr==0) {
                dW = DW;//(max_work-min_work)/((double)(DISTR_NUM_BOXES-WRIGGLE_ROOM));
                printf("#Info:  min work in sample  %.16G\n"
                       "#Info:  max work in sample  %.16G\n"
                       "#Info: dW  =  %.16G\n",min_work,max_work,dW);
            }
            //gsl_rng_set(r, gsl_rng_default_seed);
            //printf("#REWIND GSL RNG %lu\n",gsl_rng_get(r));
        }
        
        if(flag_distr_WORK) {
            //printf("WDISTR  "PART_NAME"  -1  %.16G  %d  0\n", rep_dens_work(0), cycle);
            for(ind=0;ind<DISTR_NUM_BOXES;ind++) printf("WDISTR  "PART_NAME"  %d  %.16G  %d  %.16LG  %.16G\n", ind, rep_dens_work(ind), cycle,dens_work[ind]/dW,(part.Dy)*(exp(-2.*final_t/(part.tau)) - 1.)/8.);
            //printf("WDISTR  "PART_NAME"  %d  %.16G  %d  0\n", DISTR_NUM_BOXES,rep_dens_work(DISTR_NUM_BOXES), cycle);
            printf("\n");
            printf("NEG_WORK  %d  %d  %.16G %%\n",neg_work,cycle,(double)neg_work/(double)cycle*100.);
            printf("ZERO_WORK  %d  %d  %.16G %%\n",zero_work,cycle,(double)zero_work/(double)cycle*100.);
            printf("POS_WORK  %d  %d  %.16G %%\n",pos_work,cycle,(double)pos_work/(double)cycle*100.);
            printf("\n");
            printf("WMOMEN  "PART_NAME"  ");
            for(mom=0;mom<NUM_MOMS;mom++) printf("%.16LG  ",work_cycle_mom[mom]);
            printf("\n");
            
            //printing the central moments
            long double m1,m2,m3,m4,m5;
            m1 = work_cycle_mom[1]/work_cycle_mom[0];
            m2 = work_cycle_mom[2]/work_cycle_mom[0];
            m3 = work_cycle_mom[3]/work_cycle_mom[0];
            m4 = work_cycle_mom[4]/work_cycle_mom[0];
            m5 = work_cycle_mom[5]/work_cycle_mom[0];
            printf("WMOMCENTRAL  "PART_NAME"  %d  %.16LG\n",1,m1-m1);
            printf("WMOMCENTRAL  "PART_NAME"  %d  %.16LG\n",2,m2-m1*m1);
            printf("WMOMCENTRAL  "PART_NAME"  %d  %.16LG\n",3,m3-3*m2*m1+2*m1*m1*m1);
            printf("WMOMCENTRAL  "PART_NAME"  %d  %.16LG\n",4,m4-4*m3*m1+6*m2*m1*m1-3*m1*m1*m1*m1);
            printf("WMOMCENTRAL  "PART_NAME"  %d  %.16LG\n",5,m5-5*m4*m1+10*m3*m1*m1-10*m2*m1*m1*m1+4*m1*m1*m1*m1*m1);
